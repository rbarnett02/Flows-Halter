...
void loop() {
  ...
  if (elapsedTime >= 2000) {
    ...
    publishTempbuilding(); //Publishes temp
    publishHumbuilding();   //Publishes Hum
    startTime = nowTime;
  }
}
...
void publishTempbuilding() {

  static const String topicStr = createTopic("tempbuilding");
  static const char *topic = topicStr.c_str();   
   static float tempbuilding;
    tempbuilding = dhtSensor.readTemperature(); // Reads the temperature, it takes
                                             // about 250 milliseconds

                                             


  mqttClient.publish(topic, String(tempbuilding).c_str(), RETAINED);
  Serial.println(" <= " + String(topic) + ": " + String(tempbuilding));
...
void publishHumbuilding() {

  static const String topicStr = createTopic("humbuilding");
  static const char *topic = topicStr.c_str();   
  static float humbuilding; // Variable that will store the last humidity value
  humbuilding = dhtSensor.readHumidity(); // Reads the humidity, it takes about 250
                                       // milliseconds
                                           


  mqttClient.publish(topic, String(humbuilding).c_str(), RETAINED);
  Serial.println(" <= " + String(topic) + ": " + String(humbuilding));
