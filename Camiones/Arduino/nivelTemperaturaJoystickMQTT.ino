...
//Pines del Joystick para simular GPS
const int SW_pin = 34; // digital pin connected to switch output
const int joyX = 32; // analog pin connected to X output
const int joyY = 33; // analog pin connected to Y output
int xValue, yValue;
float xMap,yMap;
...
void LecturaGPSJoystick() {

  //lectura del valor analógico del Joystick para simular GPS 
  xValue = analogRead(joyX);
  yValue = analogRead(joyY);
  xMap = map(xValue, 0,4095, 421143, 421176);
  yMap = map(yValue,0,4095,27425,27430);
  xMap = xMap/10000;
  yMap = yMap/10000;
                                            
  Serial.println(xMap, 5);
  Serial.print("\t");
  Serial.println(yMap, 5);
